...
use tracing::Span;
use tracing_opentelemetry_instrumentation_sdk::{find_trace_id_from_tracing, http as otel_http};
    ...
    B: Send + 'static,
{
    ...
    fn call(&mut self, req: Request<B>) -> Self::Future {
        ...
        let span = if self.filter.map_or(true, |f| f(req.uri().path())) {
            ...
            let traceparent = req.headers().get("traceparent").map(|h| h.to_str().unwrap()).unwrap_or_default();

            span.record("http.route", route);
            span.record("otel.name", format!("{method} {route}").trim());
            span.record("test.trace_id", find_trace_id_from_tracing(&span));
            span.record("test.traceparent", traceparent.to_string());
